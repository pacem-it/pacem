...
/// <reference path="linear-algebra.ts" />
namespace Pacem.Geometry {
    ...
    export class Utils {
        ...
        static mq(arg0: Point | Segment, p2?: Point): [number, number] {
            let p1: Point;
            if (isPoint(arg0)) {
                p1 = arg0;
            } else {
        ...
        static distance(p1: Point | Segment, arg2?: Point | [number, number]): number {
            if (isSegment(p1)) {
                return Point.distance(p1[0], p1[1]);
        ...
        static inLine(p: Point, segment: Segment): boolean {
            const v1 = Vector.from(segment[0], p),
                v2 = Vector.from(p, segment[1]);

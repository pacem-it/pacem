...
namespace Pacem.Components.Scaffolding {
    ...
    <ol class="${PCSS}-radio-list ${PCSS}-viewfinder" pacem>
        <template>
            <li><${ P }-radio disabled="{{ ::_disable.model || ^item.disabled }}" name="{{ :host.key, once }}" autobind="off" caption="{{ ^item.viewValue }}" value="{{ ^item.value }}" selected="{{ :host.isDataSourceItemSelected(^item, :host.value) }}"

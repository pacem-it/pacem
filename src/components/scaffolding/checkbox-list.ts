...
namespace Pacem.Components.Scaffolding {
    ...
    <ol class="${PCSS}-checkbox-list ${PCSS}-viewfinder" pacem>
        <template>
            <li><${ P }-checkbox disabled="{{ ::_disable.model || ^item.disabled }}" name="{{ :host.key, once }}" autobind="off" caption="{{ ^item.viewValue }}" true-value="{{ ^item.value }}" selected="{{ :host.isDataSourceItemSelected(^item, :host.value) }}"
